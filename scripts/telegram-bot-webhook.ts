...
async function main() {
  // Dynamic import — runs AFTER dotenv.config() so process.env is populated
  ...
  server.listen(PORT, () => {
    ...
    console.log(`[BTCFi Bot] Token set: ${!!process.env.TELEGRAM_BOT_TOKEN}`);
  });
}

main().catch((err) => {
  console.error('[BTCFi Bot] Fatal:', err);
  process.exit(1);
});
